  ...
  _createClass(App, [{
      ...
      if (config) {
        ...
        if (config && config.common && config.configurations) {
          ...
          $('#xsimbs').html("<tr>" + sims.map(function (sim, j) {
            var data = [j, sim.config.numberOfBuyers, sim.config.numberOfSellers];
    ...
    */
    ...
    value: function drawVisuals() {
      ...
      var withParamPlots = showCEModel === 'plot';
      var to = visual.meta.input === 'study' ? 'study-visual' : 'resultPlot';
      app.vizMaster.scaffold({
        n: app.sims.length,
        withParamPlots: withParamPlots
      });
      visual.load({
        from: app.sims,
        to: to,
    ...
    key: "zipPromise",
    value: function zipPromise(zipFile) {
      var app = this;
    ...
    value: function openZipFile(chosenSavedRun) {
      var app = this;
      var zipFile = app.study.zipFiles[+chosenSavedRun];
      $('button.openzip-button').prop('disabled', true).addClass("disabled");
      setTimeout(function () {
        (0, _singleMarketRobotSimulatorOpenzip2.default)(app.zipPromise(zipFile), app.SMRS, showZipProgress).then(function (data) {
